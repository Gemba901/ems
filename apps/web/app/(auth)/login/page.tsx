...
import { OnboardingShell, Notice } from "@/components/onboarding/OnboardingShell";
...
export default function AuthPage() {
  ...
  const [workspace, setWorkspace] = useState<{ kind: string; hostname: string } | null>(null);
  const [workspaceError, setWorkspaceError] = useState(false);
  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/workspace", { signal: AbortSignal.any([controller.signal, AbortSignal.timeout(15_000)]) })
      .then(async response => {
        if (!response.ok) throw new Error();
        setWorkspace(await response.json());
      }).catch(() => { if (!controller.signal.aborted) setWorkspaceError(true); });
