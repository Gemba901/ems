...
const ALERT_TIPS: Record<DwmsAlertField, { title: string; tips: string[]; example: string }> = {
  general: {
    ...
    example: 'Example: Packing line 2 stopped for 18 minutes due to motor overload. Production target may be missed unless maintenance checks the conveyor before next batch.',
  },
  severity: {
    title: 'Choosing severity',
    tips: ['Critical means safety, production stoppage, or major compliance risk.', 'High means meaningful business impact that needs quick attention.', 'Medium fits local issues with limited impact.'],
    example: 'Critical: line stopped. High: repeated customer dispatch delay. Medium: data mismatch needing correction.',
  },
  title: {
    title: 'Writing alert title',
    tips: ['Keep it short and specific.', 'Mention the asset, area, process, or document involved.', 'Avoid vague titles like Issue, Problem, or Alert.'],
    example: 'Good: Compressor pressure drop in Utility Area. Avoid: Machine issue.',
  },
  description: {
    title: 'Writing alert details',
    tips: ['Describe the observed abnormality and immediate impact.', 'Include time, location, quantity, equipment, or batch if relevant.', 'Mention any temporary action already taken.'],
    example: 'At 10:20 AM, Unit 3 temperature crossed 90 C for 12 minutes. Operator paused batch transfer and informed maintenance.',
  },
  target: {
    title: 'Choosing who receives it',
    tips: ['Task target links the alert to a known work item.', 'Person target is best for direct owner follow-up.', 'Department target is best when ownership is shared.', 'General is reserved for HOD or Management broadcasts.'],
    example: 'Use Department for maintenance breakdowns when the exact owner is not clear.',
  },
  task: {
    title: 'Linking a task',
    tips: ['Choose the task most closely connected to the abnormality.', 'Search by task title or responsible person.', 'If no task fits, choose Person or Department instead.'],
    example: "Link a missed checklist alert to today's assigned checklist task.",
  },
  person: {
    title: 'Selecting a person',
    tips: ['Choose the individual who can directly respond.', 'Avoid selecting someone only for awareness.', 'Use Department when responsibility is shared.'],
    example: 'Send a wrong entry alert to the person responsible for the entry correction.',
  },
  department: {
    title: 'Selecting a department',
    tips: ['Use this when the issue needs team-level ownership.', 'Pick the department closest to the process or asset.', 'Add details so the HOD can assign the right resolver.'],
    example: 'Send repeated equipment faults to Maintenance with machine ID and stoppage duration.',
  },
};

export default function DwmsAlertForm({ onCreated, onCancel }: DwmsAlertFormProps) {
  ...
  async function handleAlertSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await DwmsService.createAlert(token, {
        ...
        targetType,
        taskInstanceId: targetType === 'TASK' ? (raiseTaskInstanceId || null) : null,
        againstUserId: targetType === 'PERSON' ? (raiseAgainstUserId || null) : null,
        departmentId: targetType === 'DEPARTMENT' ? (raiseDepartmentId || null) : null,
      });
      onCreated();
    } catch (err: unknown) {
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
      <form onSubmit={handleAlertSubmit} className="space-y-6 rounded-2xl border border-border-app bg-white p-6 shadow-sm sm:p-8 lg:col-span-8">
        {message && (
        ...
        <div>
          <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-muted-app">Raised To</label>
            ...
            {[
              ...
              return (
                <button
                  ...
                  onClick={() => !disabled && setTargetType(item.value as DwmsAlertTargetType)}
                ...
                >
                  ...
                  <span className="mt-1 block text-[9px] leading-tight opacity-70">{item.desc}</span>
                </button>
              );
            })}
          </div>
        ...
        {targetType === 'TASK' && (
          <div className="relative">
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted-app">Select Task</label>
            <DwmsSelectDropdown
        ...
        {targetType === 'PERSON' && (
          <div className="relative">
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted-app">Select Person</label>
            <DwmsSelectDropdown
        ...
        {targetType === 'DEPARTMENT' && (
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted-app">Select Department</label>
            <DwmsSelectDropdown
              value={raiseDepartmentId}
              options={[
                { value: '', label: 'Choose a department...' },
                ...targetDepartments.map((d) => ({ value: d.id, label: d.name })),
              ]}
        ...
        <div className="flex justify-end gap-2 border-t border-border-app pt-4">
          <button type="button" onClick={onCancel} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-50">Cancel</button>
          <button type="submit" disabled={submitting} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-blue-700 disabled:opacity-60">
      ...
      <aside className="rounded-2xl border border-border-app bg-white p-6 shadow-sm lg:sticky lg:top-6 lg:col-span-4">
        <div className="flex items-center gap-2 border-b border-border-app pb-3">
          ...
          <h3 className="text-sm font-bold uppercase tracking-wider text-text-app">{activeTip.title}</h3>
        </div>
        <ul className="mt-5 space-y-2 text-sm leading-6 text-slate-600">
          {activeTip.tips.map((tip) => <li key={tip}>- {tip}</li>)}
        </ul>
        <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Example / suggestion</p>
          <p className="mt-2 text-sm leading-6 text-slate-700">{activeTip.example}</p>
        </div>
      </aside>
    </div>
  );
