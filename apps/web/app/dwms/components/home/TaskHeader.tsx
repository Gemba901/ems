...
type Props = {
  ...
  source: 'routine' | 'assigned';
  rightContent?: React.ReactNode;
};

export default function TaskHeader({ activeTab, setActiveTab, counts, source, rightContent }: Props) {
  const tabsList = [
  ...
  const visibleTabs = source === 'routine'
    ? tabsList.filter((tab) => tab.key !== 'NOT_ACKNOWLEDGED')
    : tabsList;

  return (
    <div className="w-full">
      {/* Sub-tabs Navigation */}
      <div className="flex items-end gap-4 border-b border-border-app">
        <div className="flex min-w-0 flex-1 gap-5 overflow-x-auto select-none whitespace-nowrap">
          {visibleTabs.map((tab) => {
