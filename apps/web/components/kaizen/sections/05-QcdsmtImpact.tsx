...
function toRows(kaizen: KaizenSectionProps["kaizen"]): Row[] {
  return kaizen.qcdsmtImpacts.map((i) => ({
    category: i.category,
...
const QcdsmtImpactSection = forwardRef<KaizenSectionHandle, KaizenSectionProps>(function QcdsmtImpactSection(
  ...
  const addRow = (category: KaizenQcdsmtCategory) => {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return KaizenService.updateQcdsmtImpact(kaizen.id, { impacts: rows.map((r) => ({ ...r, whatIsMeasured: r.whatIsMeasured.trim() })) }, token);
    },
...
export default QcdsmtImpactSection;
