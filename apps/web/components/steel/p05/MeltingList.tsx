...
const STATUS_TOOLTIPS: Record<SteelMeltingStatus, string> = {
  ...
  IN_PROGRESS: "Actively moving through the melting workflow.",
  ON_HOLD: "Paused and not currently progressing.",
  CLOSED: "Refining handover complete — melting finished.",
  CANCELLED: "This melting record was cancelled.",
};
...
export function MeltingList({
  ...
  return (
    <TooltipProvider>
      <Card>
        <CardHeader>
          <CardTitle>Melting Records</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
              <p className="text-sm text-slate-500">
                {filtersActive ? "No melting records match these filters." : "No melting records found."}
              </p>
              {filtersActive ? (
                ...
                <p className="text-xs text-slate-400">Start a new melting record against a closed charge preparation to get started.</p>
              )}
            </div>
          ) : (
            <>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div className="min-w-0 space-y-0.5">
                          ...
                          <p className="text-xs text-slate-500">
                            {melting.chargePreparation?.prepNumber ?? "—"} · Charge {melting.chargeNumberSnapshot ?? "—"}
                          </p>
                        </div>
