...
import type { SteelHeatApprovalSummary } from "@/services/steel-heat-approval.service";

interface Props {
  data?: SteelHeatApprovalSummary;
  isLoading: boolean;
  isError?: boolean;
  isFetching?: boolean;
  onRetry?: () => void;
...
// Backed by the real GET /steel/heat-approval/summary endpoint — these
// counts are the backend's exact totals, not a bounded client-side
// aggregation.
export function P06KpiCards({ data, isLoading, isError, isFetching, onRetry }: Props) {
  if (isError) {
    return (
      <Card>
        <CardContent>
